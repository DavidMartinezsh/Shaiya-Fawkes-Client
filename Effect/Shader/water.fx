...
float4x4 g_World	: WORLD; //our world view projection matrix
float4 eyePosition;
float time;
...
void main_vs(in a2v IN, out v2f OUT)
{
    ...
    OUT.bumpCoord2.xy = IN.TexCoord * textureScale * 2.0 - time * bumpSpeed * 4.0;
 
    OUT.eyeVector = P.xyz - eyePosition.xyz; // eye position in vertex space
    
    OUT.WorldPos = mul(P, g_World);
...
void main_fs(in v2f IN, out p2f OUT)
{
    ...
    R.z = -R.z;
    R.w = reflectionBlur;
    float4 reflection = texCUBE(EnvironmentMap, R.xyz) * reflectionColor;  
    //float4 reflection = lerp( texCUBE(EnvironmentMap, R.xyz), reflectionColor, LightBlendWeight );
     
	

	float specPower = 80;
	float3 EyeToPos = IN.WorldPos.xyz - eyePosition;
	
		
	float3 lightDir = {LightDirection.x*5,LightDirection.y,LightDirection.z*5};
	float3 _half = normalize( normalize(EyeToPos) + normalize(lightDir) );
	float specular = pow( dot( _half, N ), specPower ) * SpecularPower;
    ...
    // fresnel
    float facing = 1.0 - max(dot(-normalize(EyeToPos), N), 0);
    ...
    reflection = lerp(waterColor,  reflection, fresnel) * reflectionAmount;

 
    OUT.color = waterColor + reflection;
    OUT.color.rgba += (specular*reflectionColor);
